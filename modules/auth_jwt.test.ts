...
Deno.test({
  ...
  server.use((_req, ctx, next) => {
    ctx.renderToString = (vnode: any) => "<html>" + JSON.stringify(vnode) + "</html>";
    return next();
  });
  ...
  try {
    ...
    assert(signupCookie?.includes("token="), "Signup should set token cookie");
    ...
    assert(dashboardText.includes("testuser"), "Dashboard should show username");
    ...
    assert(signinCookie?.includes("token="), "Signin should set token cookie");
    ...
    assert(signoutCookie?.includes("token=;"), "Signout should clear token cookie");
    await signoutRes.body?.cancel();

    // 5. Dashboard without cookie (should redirect)
    const dashboardNoAuthRes = await fetch("http://localhost:8100/dashboard", {
      redirect: "manual",
    });
