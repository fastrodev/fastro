...
import React from "react";import { hydrateRoot } from "react-dom/client";import ${comp.toUpperCase()} from "./${comp.toLocaleLowerCase()}.page.tsx";import {atobMe as a,clean as c,extractOriginalString as ex,importCryptoKey as i,keyType,keyUsages,reverseString as rev} from "https://deno.land/x/fastro@v0.81.5/crypto/key.ts";import { decryptData as d } from "https://deno.land/x/fastro@${version}/crypto/decrypt.ts";declare global {interface Window {__INITIAL_DATA__: any;}}fetch("/__INITIAL_DATA__").then((r) => r.json()).then((v) => {let r = c(v.d) as any;let s = c(v.s) as any;s = rev(s);s = a(s);r = rev(r);r = ex(r, s);r = a(r);i(r, keyType, keyUsages).then((k) => {d(k, window.__INITIAL_DATA__).then((v) => {delete window.__INITIAL_DATA__;const p = v as any;const r = document.getElementById("root");if (r) {const props = JSON.parse(p);hydrateRoot(document.getElementById("root") as Element,<${comp.toUpperCase()} {...props} />,);}}).catch((error) => console.error(error));}).catch((error) => console.error(error));}).catch((error) => console.error(error));`;
  }
